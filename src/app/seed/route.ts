...
async function seedUser() {  
  ...
  const insertionResult = await Promise.all(user.map(async item => {
    const hashed = await bcrypt.hash(item.pw, 10);
    return sql`
    INSERT INTO "user" (id, name, join_date, pfp, email, role, password_hash)
    VALUES (${item.id}, ${item.name}, ${item.join_date}, ${item.pfp}, ${item.email}, ${item.role}, ${hashed})
    ON CONFLICT(id) DO NOTHING;`}));
  return insertionResult;
}
...
async function seedOrder() {
  ...
  const insertionResult = await Promise.all(order.map(item => sql`
    INSERT INTO "order" (id, buyer, seller, payed_at)
    VALUES (${item.id}, ${item.buyer}, ${item.seller}, ${item.payed_at})
    ON CONFLICT(id) DO NOTHING;`));
  return insertionResult;
}
...
CREATE TABLE IF NOT EXISTS "product" (
  ...
  const insertionResult = await Promise.all(product.map(item => sql`
    INSERT INTO "product" (id, shop, name, price, listed_at, description)
    VALUES (${item.id}, ${item.shop}, ${item.name}, ${item.price}, ${item.listed_at}, ${item.description})
    ON CONFLICT(id) DO NOTHING;`));
  return insertionResult;
}
...
export async function GET() {
  if(process.env.NODE_ENV === 'production' || process.env.NODE_ENV !== 'development')
    return Response.json({message: 'Seeding the database may only be done in a development environment'});

  try {
    // depends on nothing
    await sql.begin(sql => [ seedImage(), seedTag() ]);

    // depends on image
    await sql.begin(sql => [ seedUser() ]);

    // depends on user and image
    await sql.begin(sql => [ seedShop() ]);

    // depends on shop, user, and image
    await sql.begin(sql => [ seedProduct(), seedOrder() ]);

    // depends on product, order, image, shop, and user
    await sql.begin(sql => [ seedReview(), seedProductTag(), seedProductImage(), seedProductOrder() ]);

    // depends on review and image
    await sql.begin(sql => [ seedReviewImage() ]);

    return Response.json({message: 'Database seeded'});
  } catch (e) {
    console.log(e);
    return Response.json({message: 'Seeding database failed', error: e}, {status: 500});
  }
}