import { searchProductsAndGetCount } from "@/app/lib/data";
import { SuperProduct } from "@/app/lib/data";
...
async function SearchItems({ products }: { products: SuperProduct[]; }) {
  return (
    <ul>
      {products.map(product => {
        ...
        return (
          <li key={product.id}>
            ...
            <p><Link href={`shops/${product.shopId}`}>{product.shopName}</Link> | <Link href={`sellers/${product.manager}`}>{product.manager}</Link> | {product.location}</p>
